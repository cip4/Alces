...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		<html>
			...
			<body>
				...
				<div class="summary">
					...
					<strong>
						<xsl:value-of select="count(//message/tests/test[@passed='false'])=0" />
					</strong>
					<br />
					Date:
					<strong>
						<xsl:value-of select="suite/@timestamp" />
					</strong>
					<br />
					Test sessions:
					<strong>
					...
					Total tests:
					<strong>
						<xsl:value-of select="count(//message/tests/test)" />
					</strong>
					<br />
					Passed tests:
					<strong>
						<xsl:value-of select="count(//message/tests/test[@passed='true'])" />
					</strong>
					<br />
					Failed tests:
					<strong>
						<xsl:value-of select="count(//message/tests/test[@passed='false'])" />
					</strong>
					<br />
					Total outgoing messages:
					<strong>
						<xsl:value-of select="count(//message[@type='out'])" />
					</strong>
					<br />
					Total incoming messages:
					<strong>
						<xsl:value-of select="count(//message[@type='in'])" />
					</strong>
					<br />
				</div>
	...
	<xsl:template match="suite/session">
		<div class="session">
			...
			<strong>
				<xsl:value-of
					select="(count(message/tests/test[@passed='false'])+count(message/messages/message/tests/test[@passed='false']))=0" />
			</strong>
			<br />
			Total tests:
			<strong>
				<xsl:value-of
					select="count(message/tests/test)+count(message/messages/message/tests/test)" />
			</strong>
			<br />
			Passed tests:
			<strong>
				<xsl:value-of
					select="count(message/tests/test[@passed='true'])+count(message/messages/message/tests/test[@passed='true'])" />
			</strong>
			<br />
			Failed tests:
			<strong>
				<xsl:value-of
					select="count(message/tests/test[@passed='false'])+count(message/messages/message/tests/test[@passed='false'])" />
			</strong>
			<br />
			Outgoing messages:
			<strong>
				<xsl:value-of
					select="count(message[@type='out'])+count(message/messages/message[@type='out'])" />
			</strong>
			<br />
			Incoming messages:
			<strong>
				<xsl:value-of
					select="count(message[@type='in'])+count(message/messages/message[@type='in'])" />
			</strong>
			<br />
			<h3>
			...
			Header:
			<strong>
				<xsl:value-of select="message/header" />
			</strong>
			<br />
			Body:
			<a>
