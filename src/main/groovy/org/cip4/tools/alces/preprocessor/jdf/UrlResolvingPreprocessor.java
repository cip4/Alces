 ...
 * the URL is resolved against a base URL to create an absolute URL. The FileSpec/@URL is then updated with the new aboluste URL.
 * 
 * @see #BASEURL_ATTR
 * 
...
public class UrlResolvingPreprocessor implements JDFPreprocessor {
	 ...
	 */
	public final static String BASEURL_ATTR = "org.cip4.tools.alces.UrlResolvingPreprocessor.BaseURL";
	...
	/**
	 * All relative URLs (URLs witout a scheme) are resolved against the base URL and replaced with the resolved URL. If a URL cannot be resolved a warning is
	 * logged and the URL is skipped.
	...
	/**
	 * All relative URLs (URLs witout a scheme) are resolved against the base URL and replaced with the resolved absolute URL. If a URL cannot be resolved a
	 * warning is logged and the URL is skipped.
	 * 
	 * If the {@link PreprocessorContext} contains the attribute {@link #BASEURL_ATTR} it will override the base URL configured by the constructor.
	 */
	...
	/**
	 * All relative URLs (URLs without a scheme) are resolved against the base URL and replaced with the resolved URL. If a URL cannot be resolved a warning is
	 * logged and the URL is skipped.
	 * 
	 * @param jdf the JDF instance which file URLs should be resolved
	 * @param baseUrl the base URL to resolved the relative URLs against
	...
	public static JDFNode resolveReferencedFiles(final JDFNode jdf, String baseUrl) {
		...
		final List<KElement> fileSpecs = jdf.getChildrenByTagName(ElementName.FILESPEC, null, null, false, false, 0);
