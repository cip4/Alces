...
public class HttpDispatcher {
	 ...
	 * 
	 * @param maxConnections the maximum number of concurrent connections; if <code>-1</code> then the dispatcher will not be multi-threaded
	 * @param maxConnectionsPerHost the maximum number of concurrent connections per host; ignored if connectionPoolSize is <code>-1</code>
	 */
	 ...
	 * Creates a new single-threaded dispatcher for sending HTTP requests.
	 * 
	 * @param proxyHost the proxy's address; <code>null</code> if no proxy is used
	 * @param proxyPort the proxy's port; <code>-1</code> if no proxy is used
	 * @param proxyUser username for proxy authentication; <code>null</code> if authentication is not used
	 * @param proxyPassword password for proxy authentication; <code>null</code> if authentication is not used
	 */
	public HttpDispatcher(String proxyHost, int proxyPort, String proxyUser, String proxyPassword) {
	 ...
	 * @param proxyHost the proxy's address; <code>null</code> if no proxy is used
	 * @param proxyPort the proxy's port; <code>-1</code> if no proxy is used
	 * @param proxyUser Username for proxy authentication; <code>null</code> if authentication is not used. If NTLM authentication should be used,
	 * <code>proxyUser</code> should be on the format <code>domain\\username</code>.
	 * @param proxyPassword password for proxy authentication; <code>null</code> if authentication is not used
	 * @param maxConnections the maximum number of concurrent connections; if <code>-1</code> then the dispatcher will not be multi-threaded
	 * @param maxConnectionsPerHost the maximum number of concurrent connections per host; ignored if connectionPoolSize is <code>-1</code>
	 */
	public HttpDispatcher(String proxyHost, int proxyPort, String proxyUser, String proxyPassword, int maxConnections, int maxConnectionsPerHost) {
		if (maxConnections != -1) {
	...
	public InMessage dispatch(final Message message, final String url) throws IOException {
		...
		try {
			...
			final String outHeader = HttpHeaderUtils.convertHttpHeadersToString(postMethod.getRequestHeaders());
			message.setHeader(outHeader);
			// Set header for incoming message
			final String inHeader = HttpHeaderUtils.convertHttpHeadersToString(postMethod.getResponseHeaders());
			// Determines if a connection was established or not
			if (postMethod.getResponseHeaders().length != 0) {
				final Header inContentTypeHeader = postMethod.getResponseHeader(HttpHeaderUtils.CONTENT_TYPE_HEADER);
			...
			// } catch (Exception e) {
			// LOGGER.error("An error occurred while posting message to URL:" + url
			// + " ", e);
		} finally {
			if (postMethod != null) {
	...
	private PostMethod postData(Message message, String url) throws HttpException, IOException {
