...
public class XPathPreprocessor implements Preprocessor {
	...
	public static final String DEAFAULTNS_PREFIX_ATTR = "org.cip4.elk.alces.XPathPreprocessor.DefualtNSPrefix";
	
	public static final String XPATHPAIRS_ATTR = "org.cip4.elk.alces.XPathPreprocessor.XPathValuePairs";
    ...
    private static Log log = LogFactory.getLog(XPathPreprocessor.class);
    ...
    /**
     * Sets the XPaths and the corresponding value that each XPath will replace the 
     * node it selects with. 
    ...
    /**
     * Sets the prefix to use for XML documents that have a 'default namespace'.
     * All namespaces need a prefix in order to be used in XPath expressions.
     * 
    ...
    public void setDefaultNsPrefix(String defaultNsPrefix) {
        if (defaultNsPrefix == null || defaultNsPrefix.length() == 0) {
            throw new IllegalArgumentException("The default namespace " +
                    "prefix may not be null or of length zero.");
        }
        _defaultNsPrefix = defaultNsPrefix;
    }

    /**
     * Preprocesses a  message replacing all values that
     * match the configured XPath expressions with their value pairs.
     */
    public Message preprocess(Message message, PreprocessorContext context) throws PreprocessorException {     	
    	if (!(message.getContentType().startsWith(JDFConstants.JMF_CONTENT_TYPE) ||
                message.getContentType().startsWith(JDFConstants.JDF_CONTENT_TYPE) ||
                message.getContentType().startsWith(JDFConstants.XML_CONTENT_TYPE))) {
        ...
        if (_xpathValuePairs == null) {
            log.warn("The transformation will not modify the object because no" +
                    " XPaths are configured.");
            return message;
        }
    ...
    /**
     * Replaces Attribute values, Element text children, or text content that
     * matches the specified XPath.
     * 
     * The namespace prefix <code>jdf</code> is used for the namespace URI
     * <code>http://www.CIP4.org/JDFSchema_1_1</code>. Any previous namespace
     * that uses the <code>jdf</code> prefix will be replaced.
     * 
     * @param doc
     *            the document to replace values in
     * @param xpath
     *            the XPath to the nodes to replace
     * @param value
     *            the new value that replaces the previous values
     * @return a document with replaced values
     * @throws JDOMException
     */
    private Document replace(Document doc, String xpath, String defaultNsPrefix, String value)
            throws JDOMException {
        ...
        if (n.getPrefix().length() == 0) {
            ...
            log.debug("Assigned namespace prefix '" + defaultNsPrefix
                    + "' to default namespace URI '" + n.getURI() + "'.");
        } else {
            xp.addNamespace(n);
        }
        ...
        for (Iterator i = nsList.iterator(); i.hasNext();) {
            ...
            if (ns.getPrefix().length() == 0) {
                ...
                log.debug("Assigned namespace prefix '" + defaultNsPrefix
                        + "' to default namespace URI '" + ns.getURI() + "'.");
            } else {
                xp.addNamespace(ns);
            }
        }
        ...
        for (Iterator i = nodes.iterator(); i.hasNext();) {
            ...
            if (o instanceof Attribute) {
                ...
                log.warn("XPath-value pair '" + xpath + " = " + value
                        + "' was ignored because it did not select a node.");
            }
        }
        return doc;
    }
