 ...
 * 
 * <p>
 * Currently replaces existing e-mail addresses in <em>//ComChannel[@Locator='Email"]</em> by a specified or random e-mail address. To specify an e-mail
 * address, add it to the <code>
 * PreprocessorContext</code> using key {@link #EMAIL_CONTEXT_ATTR}. If no e-mail address is specified the default hostname ({@link #DEFAULT_HOSTNAME}) is used
 * with the original e-mail address as the account name.
 * </p>
 * 
...
public class ComChannelPreprocessor implements JDFPreprocessor {
	/**
	 * Attribute for storing the <em>//ComChannel[@ChannelType='Email']/@Locator</em> in the context. If this preprocessor finds this attribute in the context
	 * it will use it as a the e-mail address for all Email ComChannels found.
	 */
	public static final String EMAIL_CONTEXT_ATTR = "org.cip4.tools.alces.ComChannelPreprocessor.Email";

	/**
	 * Default e-mail address hostname: <i>cip4.example.org</i>. Used when no e-mail address is specified in the <code>PreprocessorContext</code> or no default
	 * e-mail address is specified. The hostname is prefixed by the original e-mail address with the <em>@</em> character replaced with a <em>-</em> character.
	 */
	 ...
	 * 
	 * @param defaultEmail the e-mail address that will replace the existing ones in the JDF
	 */
	...
	public JDFNode preprocess(JDFNode jdf, PreprocessorContext context) throws PreprocessorException {
		...
		for (KElement comChannelElement : comChannels) {
			...
			if (context != null && context.getAttribute(EMAIL_CONTEXT_ATTR) != null) {
