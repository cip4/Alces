...
public class TestSuiteNode extends DefaultMutableTreeNode implements TestSuite {
	...
	private static Logger LOGGER = Logger.getLogger(TestSuiteNode.class);
	 ...
	 * @see org.cip4.tools.alces.TestSuite#addTestSession(org.cip4.tools.alces.TestSessionImpl)
	 */
	public TestSession addTestSession(TestSession testSession) {
		LOGGER.debug("Adding TestSession to tree/pool: " + testSession + " " + testSession.hashCode());
		...
		Runnable addTestSession = new Runnable() {
			public void run() {
				LOGGER.debug("Inserting TestSession as child to TestSuite in tree model...");
				_treeModel.insertNodeInto(testSessionNode, thisNode, thisNode.getChildCount());
				LOGGER.debug("Inserted TestSession in tree model.");
			}
		};
		LOGGER.debug("Queueing TestSession for insertion as child to TestSuite in tree model...");
	...
	public boolean removeTestSession(TestSession testSession) {
		LOGGER.debug("Removing TestSession from tree/pool: " + testSession);
	...
	public InMessage createInMessage(String contentType, String header, String body, boolean isSessionInitiator) {
		return new InMessageNode(_wrappedTestSuite.createInMessage(contentType, header, body, isSessionInitiator), _treeModel);
	}

	public OutMessage createOutMessage(String contentType, String header, String body, boolean isSessionInitiator) {
		return new OutMessageNode(_wrappedTestSuite.createOutMessage(contentType, header, body, isSessionInitiator), _treeModel);
	}

	public TestSession createTestSession(String targetUrl) {
		return new TestSessionNode(_wrappedTestSuite.createTestSession(targetUrl), _treeModel);
	}

	public TestResult createTestResult(Test test, Message testedMessage, Result result, String testLog) {
		return new TestResultNode(_wrappedTestSuite.createTestResult(test, testedMessage, result, testLog), _treeModel);
	}
