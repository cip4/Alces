...
public class TestSessionImpl implements TestSession {
	...
	private static Log log = LogFactory.getLog(TestSessionImpl.class);
	...
	/**
	 * Creates a new test session that sends messages to the specified URL. A
	 * test session lasts for a max duration of time. After the specified
	 * duration of time has elapsed the test session is ended and any further
	 * messages received will be ignored.
	 * 
	 * @param targetUrl
	 *            the URL to send messages to during this session
	 */
	...
	public synchronized void receiveMessage(InMessage message) {
		...
		if (outMsg == null) {
			log
					.debug("No outgoing message could be found for that matches the incoming message.");
		}
	...
	/* (non-Javadoc)
	 * @see org.cip4.elk.alces.test.TestSession#getOutgoingMessage(org.cip4.elk.alces.message.InMessage)
	 */
	public synchronized OutMessage getOutgoingMessage(InMessage message) {
		...
		for (OutMessage mOut : getOutgoingMessages()) {			
			...
			if (mOut.getContentType().startsWith(JDFConstants.JMF_CONTENT_TYPE)) {
				...
				if (jmfMsgOut != null && refId.startsWith(jmfMsgOut.getID())) {
					log.debug("Found outgoing JMF message with refID '" + jmfMsgOut.getID()
							+ "' that matches incoming JMF message with refID '" + refId
							+ "'.");
					return mOut;
				}
				...
				if (tempMsg != null) {
					String mimeId = tempMsg.getBodyAsJMF().getMessageElement(null, null, 0)
							.getID();
					if (refId.startsWith(mimeId)) {
						log
								.debug("Found matching refID '" + mimeId
										+ "' in JMF MIME package.");
					}
					return mOut;
				}
		...
		log
				.warn("No outgoing message was found that matches the incoming message with refID '"
						+ refId + "'.");
		return null;
	}

	/* (non-Javadoc)
	 * @see org.cip4.elk.alces.test.TestSession#getIncomingMessage(org.cip4.elk.alces.message.OutMessage)
	 */
	/**
	 * Finds the incoming message that the specified outgoing message is a
	 * response to.
	 * 
	 * @param message
	...
	public InMessage getIncomingMessage(OutMessage message) {
		...
		try {
			...
			Namespace jdfNamespace = Namespace.getNamespace("jdf",
...
//			XPath refidXPath = XPath.newInstance("jdf:JMF/child::node()/@refID");
			...
			Attribute refIDAttr = (Attribute) refidXPath.selectSingleNode(message
					.getBodyAsJDOM());
...
//			XPath idXPath = XPath.newInstance("jdf:JMF/child::node()[@ID='" + refID + "']");
		...
		log.debug("No incoming message was found that matches the outgoing message with refID: "
		        + refID);
		return null;
	}
	...
	/**
	 * A hack that extracts a JMF message from a Message whose body is a JMF
	 * MIME package. A new Message is created using the extracted JMF message as
	 * the body.
	 * 
	 * @param message
	 *            a Message containing a JMF MIME package
	 * @return a Message containing the JMF message found in the JMF MIME
	 *         package
	 */
	public static Message getJMFFromMime(Message message) {
		...
		try {
			...
			String[] fileUrls = mimeReader.extractMimePackage(mimeStream, outputDir.toURI()
					.toURL().toExternalForm());
			// Load first file, JMF is always at first position
			for (int i = 0; i < fileUrls.length; i++) {
				if (fileUrls[i].endsWith(JDFConstants.JMF_EXTENSION)) {
					String body = IOUtils.toString(new FileInputStream(new File(new URI(
							fileUrls[i]))));
					OutMessage tempMsgOut = new OutMessageImpl(JDFConstants.JMF_CONTENT_TYPE,
							"", body, false);
        ...
        try {
            ...
            for (int i = 0; i < fileUrls.length; i++) {
                if (fileUrls[i].endsWith(JDFConstants.JDF_EXTENSION)) {
                    String body = IOUtils.toString(new FileInputStream(new File(new URI(
                            fileUrls[i]))));
                    OutMessage tempMsgOut = new OutMessageImpl(JDFConstants.JDF_CONTENT_TYPE,
                            "", body, false);
