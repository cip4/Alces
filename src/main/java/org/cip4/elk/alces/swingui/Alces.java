package org.cip4.elk.alces.swingui;
...
import org.cip4.elk.alces.jmf.JMFMessageBuilder;
...
import org.cip4.elk.alces.swingui.actions.ActionCollapse;
import org.cip4.elk.alces.swingui.actions.ActionCollapseAll;
import org.cip4.elk.alces.swingui.actions.ActionSaveRequestsResponcesToDisk;
import org.cip4.elk.alces.swingui.renderer.RendererFactory;
import org.cip4.elk.alces.swingui.tree.test.TestSuiteNode;
import org.cip4.elk.alces.test.TestResult;
import org.cip4.elk.alces.test.TestRunner;
import org.cip4.elk.alces.test.TestSession;
import org.cip4.elk.alces.util.ConfigurationHandler;
import org.cip4.elk.alces.util.JDFFileFilter;
import org.cip4.elk.alces.util.JMFFileFilter;
...
public class Alces extends JFrame implements ActionListener, TreeModelListener, TreeSelectionListener, MouseListener
{
	...
	private static Log log = LogFactory.getLog(Alces.class);
	...
	/**
	 * Creates a new instance of the Alces Swing application using the specified
	 * locale.
	 * 
	 * @param locale
	 *            the <code>Locale</code> used for labels in the user
	 *            interface
	 * @throws Exception
	 */
	public Alces() throws Exception
	{
		...
		deviceInfoPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(5, 6, 0, 6), BorderFactory.createTitledBorder(_confHand.getLabel("known.devices", "Known Devices"))), BorderFactory.createEmptyBorder(0, 5, 5, 5)));
		devicePanel.add(deviceInfoPanel, BorderLayout.NORTH);
		// Known Messages
		messagesPanel = new JPanel();
		messagesPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(5, 6, 6, 6), BorderFactory.createTitledBorder(_confHand.getLabel("known.messages", "Known Messages"))), BorderFactory.createEmptyBorder(0, 5, 5, 5)));
		messagesPanel.setLayout(new BoxLayout(messagesPanel, BoxLayout.Y_AXIS));
		devicePanel.add(messagesPanel, BorderLayout.CENTER);

		//		Batch Panel
		JPanel batchPanel = new JPanel();
		batchPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(5, 6, 6, 6), BorderFactory.createTitledBorder("Batch Mode")), BorderFactory.createEmptyBorder(0, 5, 5, 5)));
	...
	/**
	 * Sends a JMF message containing a Status message with a QueueInfo='true'
	 * and QueueStatus message.
	 * 
	 * @throws IOException
	 */
	 ...
	 * 
	 * @param knownDevices
	 *            a JMF message containing a KnownDevices response
	 */
	...
	/**
	 * Sets which device to display device information about and to send
	 * messages to.
	 * 
	 * @param idx
	 *            the index of the device in the combobox
	 */
	...
	/**
	 * Sends a KnownMessages query to the configured device and populates the UI
	 * with the buttons representing all messages the device supports.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Adds buttons to the GUI based on the message services specificed in the
	 * JMF response. If the JMF response does not contain any message services a
	...
	public void buildMessageButtons(JDFJMF knownMessages)
	{
		...
		// Create SubmitQueueEntry button if there was no JMF response
	 ...
	 * 
	 * @param text
	 *            the text on the button, should be the message's type
	 * @param toolTip
	 *            a help message
	 * @param actionCommand
	 *            the message's xsi:type
	 * @return
	 */
	...
	/**
	 * Returns an instance of a <code>Message</code> generated from the
	...
	/**
	 * Doing the init stuff for the Tree, it's Renderer and Model Sets the
	 * TreeModel in the ConfigurationHandler
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param deviceInfo
	 *            the DeviceInfo to update the GUI with
	 */
	...
	/**
	 * Cancels that connection attempt by calling the thread executing the
	 * connect.
	 * 
	 * @see #connect()
	 */
	...
	/**
	 * Connects Alces to the device/controller specified by the URL in the
	 * address bar. Connecting consists of the following phases:
	 * 
	 * 1. Clear GUI with information about previous device 2. Initialize a new
	 * test invironment -> Abort if error occurs 3. Send KnownDevice handshake ->
	 * Display warning if error occurs 4. Send KnownMessages handshake ->
	 * Display warning if error occurs 5. Update GUI with device information
	 * 
	 * A separate thread is created for executing the connect. If the user
	 * cancels connecting, the thread is left orphaned and will self-terminate
	 * when it completes the phase it was in when the user cancelled.
	 * 
	 * @see #cancelConnect()
	 */
			...
			public void run()
			{
				...
				catch (Exception e)
				{
					...
					JOptionPane.showMessageDialog(Alces.this, "Alces test envirnement could not be initialized\n" + "prior to connecting. View the log file 'alces.log'\n"
							+ "for details.", "Could Not Connect", JOptionPane.ERROR_MESSAGE);
					return;
				}
				// Send JMF handshake
				try
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (actionCommand.equals(ACTION_CONNECT))
		{
			connect();
		}
		else if (actionCommand.equals(ACTION_CONNECT_CANCEL))
		{
			cancelConnect();
		}
		else if (actionCommand.equals(ACTION_SEND_FILE))
		{
			// Send file to device
			final JFileChooser fc = new JFileChooser(_confHand.getProp(ConfigurationHandler.LAST_DIR));
			fc.addChoosableFileFilter(new JMFFileFilter());
			...
			_confHand.putProp("last.dir", fc.getCurrentDirectory().getAbsolutePath());
			if (returnVal == JFileChooser.APPROVE_OPTION)
			{
				testRunner.startTestSession(testRunner.loadMessage(fc.getSelectedFile()), getDeviceUrl());
			}
		}
		else if (actionCommand.equals(ACTION_BATCH_SELECT_FILE))
		{
			JFileChooser fc = new JFileChooser(_confHand.getProp(ConfigurationHandler.LAST_DIR));
			fc.setFileSelectionMode(JFileChooser.FILES_ONLY);
			fc.addChoosableFileFilter(new JMFFileFilter());
			fc.setAcceptAllFileFilterUsed(true);
			fc.setDialogTitle("Batch Mode: Select a File to Send");
			int returnVal = fc.showOpenDialog(this);
			...
			if (returnVal == JFileChooser.APPROVE_OPTION)
			{
				...
				batchModeLabel.setText(fc.getSelectedFile().getAbsolutePath());

				filesToSendInBatch.clear();
				filesToSendInBatch.add(fc.getSelectedFile());
			}
		}
		else if (actionCommand.equals(ACTION_BATCH_SELECT_FOLDER))
		{
			JFileChooser fc = new JFileChooser(_confHand.getProp(ConfigurationHandler.LAST_DIR));
			fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			fc.addChoosableFileFilter(new JMFFileFilter());
			fc.setAcceptAllFileFilterUsed(true);
			fc.setDialogTitle("Batch Mode: Select a Folder to Send");
			int returnVal = fc.showOpenDialog(this);
		...
		else if (actionCommand.equals(ACTION_BATCH_START))
		{
		...
		else if (actionCommand.equals(ACTION_BATCH_STOP))
		{
			log.info(ACTION_BATCH_STOP);
			batchStopButton.setEnabled(false);
			isBatchRunned = false;
		}
		else if (actionCommand.startsWith("Command") || actionCommand.startsWith("Query"))
		{
			if (actionCommand.equals("CommandSubmitQueueEntry"))
			{
				final JFileChooser fc = new JFileChooser(_confHand.getProp(ConfigurationHandler.LAST_DIR));
				fc.addChoosableFileFilter(new JDFFileFilter());
				fc.setDialogTitle("Select a JDF Job Ticket to Submit");
				final boolean packageAsMime = (e.getModifiers() & ActionEvent.ALT_MASK) != 0;
				if (packageAsMime)
				{
					fc.setDialogTitle(fc.getDialogTitle() + " in a MIME package");
				}
				final boolean disablePreprocessing = (e.getModifiers() & ActionEvent.SHIFT_MASK) != 0;
				if (disablePreprocessing)
				{
					fc.setDialogTitle(fc.getDialogTitle() + " - JDF preprocessing is disabled");
				}
				int returnVal = fc.showOpenDialog(this);
				_confHand.putProp("last.dir", fc.getCurrentDirectory().getAbsolutePath());
				if (returnVal == JFileChooser.APPROVE_OPTION)
				{
					testRunner.startTestSessionWithSubmitQueueEntry(fc.getSelectedFile(), getDeviceUrl(), !disablePreprocessing, packageAsMime);
				}
				return;
			}
			else if (actionCommand.equals("CommandResubmitQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				final String jobID = queuePanel.getSelectedJobID();
				if (queueEntryID != null && jobID != null)
				{
					final JFileChooser fc = new JFileChooser(_confHand.getProp(ConfigurationHandler.LAST_DIR));
					fc.addChoosableFileFilter(new JDFFileFilter());
					fc.setDialogTitle("Select a JDF Job Ticket to Resubmit");
					final boolean packageAsMime = (e.getModifiers() & ActionEvent.ALT_MASK) != 0;
					if (packageAsMime)
					{
						fc.setDialogTitle(fc.getDialogTitle() + " in a MIME package");
					...
					if (returnVal == JFileChooser.APPROVE_OPTION)
					{
						testRunner.startTestSessionWithResubmitQueueEntry(fc.getSelectedFile(), queueEntryID, jobID, getDeviceUrl(), !disablePreprocessing, packageAsMime);
					}
				}
				else
				{
					JOptionPane.showMessageDialog(this, _confHand.getLabel("please.select.row", "Please select a queue entry in the queue table"), _confHand.getLabel("No Row Selected", "No Row Selected"), JOptionPane.WARNING_MESSAGE);
				}
				return;
			}
			else if (actionCommand.equals("CommandStopPersistentChannel"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				final String jobID = queuePanel.getSelectedJobID();
				OutMessage message = JMFMessageBuilder.buildStopPersistentChannel(_confHand.getServerJmfUrl(), queueEntryID, jobID);
				testRunner.startTestSession(message, getDeviceUrl());
				return;
			}
			else if (actionCommand.equals("CommandAbortQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildAbortQueueEntry(queueEntryID);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandHoldQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildHoldQueueEntry(queueEntryID);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandRemoveQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildRemoveQueueEntry(queueEntryID);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandResumeQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildResumeQueueEntry(queueEntryID);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandSuspendQueueEntry"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildSuspendQueueEntry(queueEntryID);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandSetQueueEntryPriority"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildSetQueueEntryPriority(queueEntryID, 100);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("CommandSetQueueEntryPosition"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				if (queueEntryID != null)
				{
					OutMessage message = JMFMessageBuilder.buildSetQueueEntryPostion(queueEntryID, 0, null, null);
					testRunner.startTestSession(message, getDeviceUrl());
				}
				else
				{
			...
			else if (actionCommand.equals("QueryStatus"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				final String jobID = queuePanel.getSelectedJobID();
				if (queueEntryID != null)
			...
			else if (actionCommand.equals("QueryResource"))
			{
				final String queueEntryID = queuePanel.getSelectedQueueEntryID();
				final String jobID = queuePanel.getSelectedJobID();
				OutMessage message = JMFMessageBuilder.buildQueryResource(jobID, queueEntryID);
				testRunner.startTestSession(message, getDeviceUrl());
				return;
			}
			// TODO Allow possibility to add subscription
			// TODO Allow manual modification of messages
			testRunner.startTestSession(createMessage(actionCommand), getDeviceUrl());
		}
		else if (actionCommand.equals(QueuePanel.REFRESH_QUEUE))
		{
			JDFJMF jmf = null;
			try
			{
		...
		else if (actionCommand.equals(ACTION_SELECT_DEVICE))
		{
	...
	class AlcesTreeCellRenderer extends DefaultTreeCellRenderer
	{
		...
		{
			sessionIcon = createIcon("/org/cip4/elk/alces/icons/session.gif");
			messageInPassIcon = createIcon("/org/cip4/elk/alces/icons/message_in_pass.gif");
			messageInFailIcon = createIcon("/org/cip4/elk/alces/icons/message_in_fail.gif");
			messageOutPassIcon = createIcon("/org/cip4/elk/alces/icons/message_out_pass.gif");
			messageOutFailIcon = createIcon("/org/cip4/elk/alces/icons/message_out_fail.gif");
			testPassIcon = createIcon("/org/cip4/elk/alces/icons/test_pass.gif");
			testFailIcon = createIcon("/org/cip4/elk/alces/icons/test_fail.gif");
			testIgnoredIcon = createIcon("/org/cip4/elk/alces/icons/test_ignored.gif");
			sessionFailIcon = createIcon("/org/cip4/elk/alces/icons/session_fail.gif");
			sessionPassIcon = createIcon("/org/cip4/elk/alces/icons/session_pass.gif");
		}
	...
	private void quitAlces()
	{
		...
		try
		{
			outputFile = testRunner.serializeTestSuite(_confHand.getProp(ConfigurationHandler.XSLT_REPORT_DIR), outputDir);
