...
import org.cip4.elk.alces.test.TestSuiteImpl;
...
public class Alces {
	...
	public static void main(String[] args) throws Exception {
		...
		if (exit) {
			...
			System.out
					.println("  <testdata>     Path to the file or directory to send");
			System.out
					.println("  <properties>  Path to a properties file that overrides the default properties file (../conf/alces.properties)");
			System.out.println();
	...
	public static int runTestdataScript(String url, File testdata, File propsFile)
			throws Exception {
	 ...
	 */
	@Deprecated
	public static int runTestdata(File testdataDir, String url, String propsFile)
			throws Exception {
		...
		try {
			...
			String outputDir = config.getProp(ConfigurationHandler.OUTPUT_DIR) + " "
					+ dateFormat.format(new Date());
			log.info("Writing test suite to '" + outputDir + "'...");
			String outputFile = testRunner.serializeTestSuite(config.getProp(ConfigurationHandler.XSLT_REPORT_DIR), outputDir);
	...
	public static int runScript(File scriptFile, String url, File testdata,
		...
		try {
			...
			// Evaluate script
			Object result = cx.evaluateReader(scope, new FileReader(scriptFile), scriptFile
					.getName(), 0, null);
	 ...
	 * 
	 * @param xmlPath
	 *            the absolute path to the XML document to transform
	 * @param xslPath
	 *            the absolute path to the XSL stylesheet to use
	 */
