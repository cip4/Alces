...
public abstract class AbstractMessage implements Message {
	...
	protected static Log log = LogFactory.getLog(AbstractMessage.class);
	 ...
	 * @param contentType
	 * @param header
	 *            The <code>Message</code>'s header. This usually corresponds
	 *            to the HTTP header fields.
	 * @param body
	 *            the content of the <code>Message</code>'s body
	 * @param isSessionInitiator
	 *            true if this <code>Message</code> initiated a
	 *            <code>TestSession</code>
	 */
	public AbstractMessage(String contentType, String header, String body,
	...
	/**
	 * Creates a new Message. The content-type defaults to the JMF content-type
	...
	public String toString() {
		return "Message[ ContentType=" + _contentType + "; Header=" + _header + "; Body="
				+ _body + " ]";
	}
	 ...
	 * 
	 * @return a JDOM tree; <code>null</code> if the message does not contain
	 *         XML
	 */
	public org.jdom.Document getBodyAsJDOM() {
		...
		try {
			...
			log.debug("Could not build JDOM from message body.", e);
		}
		return doc;
	}
	 ...
	 * 
	 * @return the JMF message; <code>null</code> if the message does not
	 *         contain JMF
	 */
	public JDFJMF getBodyAsJMF() {
		...
		try {
			...
			log.debug("Could not build JMF from message body.", e);
		}
		return jmf;
	}
	 ...
	 * 
	 * @return the JDF instance; <code>null</code> if the message does not
	 *         contain JDF
	 */
	public JDFNode getBodyAsJDF() {
		...
		try {
			...
			log.debug("Could not build JDF from message body.", e);
		}
		return jdf;
	}
	...
	public void addTestResult(TestResult testResult) {
		log.debug("Adding TestResult to Message...");
