...
/**
 * Requires that 'user.dir' is set to 'Alces/src/bin' for CheckJDFWrapper's path
 * to the JDF schema to be correct.
 * 
...
public class CheckJDFWrapperTest extends AlcesTestCase {
	...
	private static final String TEST_DATA = "src/test/data/CheckJDFWrapperTest/";

	@Override
	public void setUp() throws Exception {
		super.setUp();
		/*
		 * String userDir = System.getProperty("user.dir"); if
		 * (!userDir.endsWith("/src/bin")) { userDir =
		 * System.setProperty("user.dir", userDir + "/src/bin"); }
		 */
	...
	public void testValidateInputStreamFile_JMF() throws Exception {
		...
		for (int i = 0; i < _jmfFiles.length; i++) {
			boolean result = CheckJDFWrapper.validate(new FileInputStream(_jmfFiles[i]),
					reportFile);
			assertTrue("Validation failed: " + i + ". " + _jmfFiles[i], result);
			assertTrue("Report missing for JMF file " + i + ": " + _jmfFiles[i], reportFile
					.exists());
			reportFile.delete();
		}
	}
	...
	public void disabled_testValidateFileFileFileFile_JMF() throws Exception {
		...
		assertTrue(schemaFile.exists());
		File devcapFile = new File(TEST_DATA + "devcaps/DevCaps_LayCrImp.jdf")
				.getCanonicalFile();
		assertTrue(devcapFile.exists());
		File reportFile = File.createTempFile("report", ".xml");
		reportFile.delete();
		for (int i = 0; i < _jmfFiles.length; i++) {
			boolean result = CheckJDFWrapper.validate(_jmfFiles[i], schemaFile, devcapFile,
					reportFile);
			assertTrue("Validation failed: " + _jmfFiles[i], result);
			assertTrue(reportFile.exists());
			reportFile.delete();
	...
	public void testValidateApprovalDevCap() throws Exception {
		File jdfFile = new File(TEST_DATA + "devcaps/Elk_Approval_DevCaps.jmf")
				.getCanonicalFile();
	...
	public void testValidateApprovalJDF_ApprovalDevCap_NoSchema() throws Exception {
		...
		File schemaFile = null;
		// Approval DevCap
		File devcapFile = new File(TEST_DATA + "devcaps/Elk_Approval_DevCaps.jmf")
				.getCanonicalFile();
		assertTrue(devcapFile.exists());
		// Report
		File reportFile = File.createTempFile("report", ".xml");
	...
	/**
	 * Validates a JMF message with and explicitly stated JDF Schema. The schema
	 * used is the schema bundled with Alces.
	 */
	public void testValidateWithSchema() throws Exception {
		...
		File devcapFile = null;
		File reportFile = null;
		boolean result = CheckJDFWrapper.validate(_jmfFiles[0], schemaFile, devcapFile,
				reportFile);
		assertTrue(result);
	}
	...
	/**
	 * Validates a JMF message with and explicitly stated JDF Schema. The schema
	 * used is a temporary copy of the schema bundled with Alces. The only
	 * difference between the temporary copy and the origianl is that the copy's
	...
	public void testValidateWithSchema_SpacesInFilename() throws Exception {
		...
		boolean result = CheckJDFWrapper.validate(_jmfFiles[0], newSchemaFile, devcapFile,
				reportFile);
	...
	/**
	 * A filter that filters out directories first and then applies the filter
