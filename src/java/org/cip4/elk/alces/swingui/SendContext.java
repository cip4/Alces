...
public class SendContext extends JPopupMenu implements MouseListener {
	...
	private static Log log = LogFactory.getLog(SendContext.class);
	...
	/**
	 * Simple constructor <code>source</code> is used to call the JOptionPane.
	 * <code>type</code> describes if the context is called for send JMF or
	 * SubmitQueueEntry
	 * 
	 * @param source
	 * @param type
	...
	public SendContext(Alces source, String type) {
		...
		label = new JMenuItem(_confHand.getLabel("Select file to send:",
				"Select file to send:"));
		this.add(label);
		sep = new JSeparator();
		this.add(sep);
	...
	private void createJMFItems() {
		...
		for (int i = 0; i < testFile.length; i++) {
			if (new JMFFileFilter().acceptFilesOnly(new File(_dir + ""
		...
		try {
			for (int i = 0; i < _entries.size(); i++) {
				...
				try {
					fileItem.setToolTipText((new File(_dir)).getCanonicalPath()
							+ "" + File.separator + ""
							+ _entries.get(i).toString());
				} catch (IOException e) {
			...
			JOptionPane
					.showMessageDialog(
							_source,
							_confHand
									.getLabel("Error Directory",
											"No directory found! Please set a default directory in alces.properties."),
	...
	private void createJDFItems() {
		...
		for (int i = 0; i < testFile.length; i++) {
			if (new JDFFileFilter().acceptFilesOnly(new File(_dir + ""
					+ File.separator + "" + testFile[i])))
	...
	public void mousePressed(MouseEvent e) {
		...
		if (comp.getName().endsWith(".jmf")) {
			_source.loadContextMessage(new File(_dir + "" + File.separator + ""
					+ comp.getName()));
		}
		if (comp.getName().endsWith(".jdf")) {
			_source.loadContextJDF(new File(_dir + "" + File.separator + ""
					+ comp.getName()));
		}
	}
