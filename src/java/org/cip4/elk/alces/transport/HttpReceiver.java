...
import org.cip4.elk.alces.test.TestSuite;
...
public class HttpReceiver {
    ...
    private static Log log = LogFactory.getLog(HttpReceiver.class);
    ...
    /**
     * Creates a new HttpReceiver. Received messages are added to the looked the
     * specified <code>TestSuite</code>.
     * 
     * @param testSuite
     *            the <code>TestSuite</code> to add received messages to
     */
    ...
    /**
     * Starts a server that listens to requests at:
     * <code>http://127.0.0.1:<strong>port</strong>/alces/jmf</code>
     * 
     * @param port
     * @throws Exception
     */
    ...
    /**
     * Starts a server that listens for requests at:
     * <code>http://<strong>host</strong>:<strong>port</strong>/alces/jmf</code>
     * 
     * @param host
     * @param port
     * @throws Exception
    ...
    /**
     * Starts a HTTP server that publishes static files, such as JDF and
     * resource files, to <code>http://<em>host</em>:<em>port</em>/</code>
     * and listens for JMF requests on
     * <code>http://<em>host</em>:<em>port</em>/<em>contextPath</em></code>.
     * 
     * @param host
     *            server hostname
     * @param port
     *            server port
     * @param jmfContextPath
     *            server context path where server listens for JMF messages
     * @param resourceBase
     *            path in local filesystem to the root folder of the static
     *            files that will be published be the server
     * @throws Exception
     *             if the HTTP server could not by started
     */
    public synchronized void startServer(String host, int port,
            String jmfContextPath, String resourceBase) throws Exception {
        ...
        if (_server != null && _server.isStarted()) {
            try {
                log
                        .debug("Stopping active HTTP server before starting a new one.");
                stopServer();
        ...
        ServletHolder holder = servlets.addServlet(jmfContextPath,
                "org.cip4.elk.alces.transport.JMFServlet");
