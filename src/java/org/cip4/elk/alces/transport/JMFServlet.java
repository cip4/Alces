 ...
 * @version $Id$
 * 
 * @web:servlet name="JMFServlet" display-name="Alces JMF Servlet"
...
public class JMFServlet extends HttpServlet {
    ...
    /**
     * Entry point that delegates to the process methods based on the request
...
//                processMime(req, res);
//            } else {
//                log.debug("Attempting to receive message with content-type '" + 
...
//        }
     ...
     * @throws IOException
     */
    public void processMessage(HttpServletRequest req, HttpServletResponse res)
            throws IOException {
        ...
        if (contentType == null &&
            new Boolean(_confHand.getProp(ConfigurationHandler.NO_CONTENT_TYPE)))
        {
            contentType = JMFServlet.JMF_CONTENT_TYPE;
        }
        final String header = HttpHeaderUtils.convertHttpHeadersToString(req);
        final InMessage message = _testSuite.createInMessage(contentType, header,
                messageBody, false);
        ...
        if (jmfIn != null) {
        	...
        	res.sendError(HttpServletResponse.SC_UNSUPPORTED_MEDIA_TYPE, 
                    "The request's content-type was: " + contentType);
        }
    }
...
//    }
     ...
     * @param message
     */
    private void startTestSession(HttpServletRequest req,
            @SuppressWarnings("unused")
			HttpServletResponse res, InMessage message) {
//        _confHand = ConfigurationHandler.getInstance();
        ...
        if (_testSuite != null) {
            ...
            if (testSession != null) {
                ...
                InMessage newMessage = _testSuite.createInMessage(message
                        .getContentType(), message.getHeader(), message
                        .getBody(), true);
                testSession = _testSuite.createTestSession(req
                        .getRemoteAddr());
