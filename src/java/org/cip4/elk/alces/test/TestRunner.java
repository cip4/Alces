...
import org.cip4.elk.alces.preprocessor.jdf.JobIDPreprocessor;
import org.cip4.elk.alces.preprocessor.jdf.NodeInfoPreprocessor;
import org.cip4.elk.alces.preprocessor.jdf.UrlResolvingPreprocessor;
import org.cip4.elk.alces.preprocessor.jmf.Preprocessor;
import org.cip4.elk.alces.preprocessor.jmf.SenderIDPreprocessor;
import org.cip4.elk.alces.preprocessor.jmf.URLPreprocessor;
...
import org.cip4.elk.alces.util.NotDirFilter;
...
/**
 * This is the class in the Alces framework that is responsible for creating
 * test sessions during which JMF messages are sent, received, and tested.
 * Example usage:
 * <ol>
 * <li>Create a TestRunner, {@link #TestRunner()}.</li>
 * <li>Initialize the test environment {@link #init()}. This starts a HTTP
 * server, etc.</li>
 * <li>Start a {@link TestSession} using one of the
 * <code>startTestSession*</code>-methods.</li>
 * <li>Any incoming messages that Alces can associate with the outgoing message
 * that started a <code>TestSession</code> will automatically be associated
 * with the <code>TestSession</code>. To receive events when an incoming
 * message is received, register a listener with the <code>TestSession</code>
 * using {@link TestSession#addListener(TestSessionListener)}</li>
 * <li>When finished, destroy the test environment to release resources,
 * {@link #destroy()}</li>
 * </ol>
 * 
...
public class TestRunner {
	...
	/**
	 * Default constructor that uses the {@link TestSuite} implemenation
	 * {@link TestSuiteImpl}.
	 */
	public TestRunner() {
	...
	/**
	 * Creates a new test environment for running tests. The {@link TestSuite}
	 * is the data model used to store all test data that is generated during
	 * the test sessions ({@link TestSession}).
	 * 
	 * @param testSuite
	 *            the TestSuite implementation to which all test data will
	 *            generated by the TestRunner will be added
	 * @see TestSuite
	 * @see TestSession
	 */
	 ...
	 * 
	 * @param targetUrl
	 *            the URL to send the test data to
	 * @param testDataDir
	...
	public void runTests(String targetUrl, String testDataDir) throws Exception {
		...
		try {
			int sleepTime = Integer.parseInt(_confHand
					.getProp(ConfigurationHandler.SESSION_DURATION));
	 ...
	 * 
	 * @param testFile
	 *            the path to a file containing a message that will start the
	 *            test session
	 * @param targetUrl
	...
	/**
	 * Starts a new test session using the specified file as the test session's
	 * initiating outgoing message.
	 * 
	 * All outgoing messages sent and incoming messages received during a test
	 * session are tested by the {@link org.cip4.elk.alces.test.tests.Test}s
	 * configured for this <code>TestRunner</code>.
	 * 
	 * All outgoing messages are preprocessed by the
	 * {@link org.cip4.elk.alces.preprocessor.jmf.Preprocessor}s configured for
	 * this <code>TestRunner</code>.
	 * 
	 * @param testFile
	 *            a file containing the message that starts the test session,
	 *            for example a JMF message or a MIME package
	 * @param targetUrl
	 *            the URL to send the outgoing message to
	 * @return the started test session
	 */
	public TestSession startTestSession(File testFile, String targetUrl) {
		return startTestSession(loadMessage(testFile), null, new PreprocessorContext(),
				targetUrl);
	}

	/**
	 * Starts a <code>TestSession</code> based on the specified outgoing
	 * message.
	 * 
	 * @param outMessage
	 *            the outgoing message that starts the test session
	 * @param jdfFile
	 *            the JDF file submitted by the message; <code>null</code> if
	 *            no JDF file was submitted
	 * @param context
	 *            the context used when preprocessing the message and the JDF
	 *            file
	 * @param targetUrl
	 *            the URL to send the outgoing message to
	 * @return the started test session
	 */
	public TestSession startTestSession(OutMessage outMessage, File jdfFile,
			PreprocessorContext context, String targetUrl) {
	 ...
	 * @param asMime
	 *            <code>true</code> to package the JDF file, its content files, and the
	 *            SubmitQueueEntry JMF message in a MIME package
	 * @return
	 */
	public TestSession startTestSession(OutMessage outMessage, File jdfFile,
			PreprocessorContext context, String targetUrl, boolean asMime) {
		// Preprocess message
		context.addAttribute(SenderIDPreprocessor.SENDERID_ATTR, _confHand
				.getProp(ConfigurationHandler.SENDER_ID));
		...
		// Preprocess JDF
		if (jdfFile != null) {
			context.addAttribute(NodeInfoPreprocessor.SUBSCRIPTION_URL_ATTR, _confHand
					.getServerJmfUrl());
			context.addAttribute(NodeInfoPreprocessor.MESSAGEID_PREFIX_ATTR, outMessage
					.getBodyAsJMF().getMessageElement(null, null, 0).getID());
	...
	/**
	 * Packages an the JMF message contained in <code>outMessage</code>, the
	 * JDF file, and all files referenced by the JDF file in a MIME package that
	 * is wrapped in an <code>OutMessage</code>.
	 * 
	 * @param outMessage
	 *            the JMF message to package together with the JDF file
	 * @param jdfFile
	 *            the JDF file to package
	 * @return an OutMessage which body is the MIME package
	 * @todo Refactor! We really need a smarter base implementation of Message
	 *       that does no hold the entire message in memory. MIME packages can
	 *       be huge...
	 */
	public OutMessage packageAsMime(OutMessage outMessage, File jdfFile) {
		...
		if (jdfFile instanceof PublishedFile) {
			jdfDoc.setOriginalFileName(((PublishedFile) jdfFile).getOriginalFile()
					.getAbsolutePath());
		}
	...
	private HttpDispatcher configureHttpDispatcher() {
		int maxConnections;
		try {
			maxConnections = Integer.parseInt(_confHand
					.getProp(ConfigurationHandler.OUTGOING_CONNECTIONS));
		...
		if (Boolean.parseBoolean(_confHand.getProp(ConfigurationHandler.PROXY_ENABLED))) {
			...
			dispatcher = new HttpDispatcher(
					proxyHost, proxyPort, 
					_confHand.getProp(ConfigurationHandler.PROXY_USER), 
					_confHand.getProp(ConfigurationHandler.PROXY_PASSWORD), 
	 ...
	 * 
	 * @param testdataPath
	 *            the path to a test file or directory containing test files to
	 ...
	 * 
	 * @throws Exception
	 *             if the test session could not be initialized
	 * @see #destroy()
	 */
	public void init() throws Exception {
		...
		try {
			...
			try {
				...
				log
						.warn(
								"Could not stop HTTP receiver while aborting test environment initialization.",
								e1);
			}
			_httpReceiver = null;
			_dispatcher = null;
	...
	/**
	 * Sends a <code>OutMessage</code> to the preconfigured target URL. The
	 * message is preprocessed before it is sent.
	 * 
	 * If <code>SenderIDPreprocessor</code> is enabled then
	 * <em>JMF/@SenderID</em> will be replaced with the value configured
	 * SenderID.
	 * 
	 * @param message
	 *            the <code>InMessage</code> received in the response
	 * @return
	 * @throws IOException
	 *             if an communication exception occurs during the message
	 *             sending
	 */
	...
	/**
	 * Starts a new test session by resubmitting a JDF file. Any relative URLs
	 * in the JDF file will be resolved against the JDF file and replaced with
	 * http URLs.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 * @param targetUrl
	 *            the URL to submit it to
	 * @param preprocessJdf
	 *            <code>true</code> to preprocess the JDF file before
	 *            submitting it; <code>false</code> otherwise
	 * @return the test session
	 * @see #startTestSessionWithResubmitQueueEntry(File, String, String,
	 *      String)
	 * @see #publishJDFWithFileUrls(File, String)
	 */
	public TestSession startTestSessionWithSubmitQueueEntryWithHttpUrls(File jdfFile,
			String targetUrl, boolean preprocessJdf) {
		return startTestSessionWithSubmitQueueEntry(jdfFile, targetUrl, preprocessJdf, false,
				getPublishedJDFBaseURL());
	}
	 ...
	 *            the URL to submit it to
	 * @param preprocessJdf
	 *            if <code>true</code> the JDF file is preprocessed before
	 *            sent
	 * @return the test session
	 * @see #startTestSessionWithResubmitQueueEntry(File, String, String,
	 *      String)
	 * @see #publishJDFWithFileUrls(File, String)
	 */
	public TestSession startTestSessionWithSubmitQueueEntryWithFileUrls(File jdfFile,
			String targetUrl, boolean preprocessJdf) {
		return startTestSessionWithSubmitQueueEntry(jdfFile, targetUrl, preprocessJdf, false,
				jdfFile.getParentFile().toURI().toASCIIString());
	}

	/**
	 * Starts a new test session by sending a SubmitQueueEntry JMF message that
	 * refers to the specified JDF file. Before submitting the JDF job, the JDF
	 * file is published to this TestRunners HTTP server and the resulting http
	 * URL is used in the SubmitQueueEntry JMF message.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 * @param targetUrl
	 ...
	 *            submitting it; <code>false</code> otherwise
	 * @param asMime
	 *            packages the JDF file, its content files, and the
	 *            SubmitQueueEntry JMF in a MIME package
	 * @param baseUrl
	 *            If the JDF file references any files using relative URLs then
	 *            these will be resolved against this base URL.
	 *            <code>null</code> to not resolve any URLs.
	 * @return the test session
	 */
	public TestSession startTestSessionWithSubmitQueueEntry(File jdfFile, String targetUrl,
			boolean preprocessJdf, boolean asMime, String baseUrl) {
		jdfFile = publishJDF(jdfFile);
		PreprocessorContext context = new PreprocessorContext();
		context.addAttribute(JDFPreprocessor.PREPROCESSING_ENABLED, Boolean
				.toString(preprocessJdf));
		context.addAttribute(UrlResolvingPreprocessor.BASEURL_ATTR, baseUrl);
		// String jdfUrl = publishJDF(jdfFile, replaceJobId, baseUrl);
		OutMessage message = JMFMessageBuilder
				.buildSubmitQueueEntry(resolvePublishedJDF(jdfFile));
	 ...
	 */
	public TestSession startTestSessionWithSubmitQueueEntry(File jdfFile, String targetUrl,
			boolean preprocessJdf, boolean asMime) {
		...
		if (replaceUrls.equals(ConfigurationHandler.REPLACE_URLS_IN_JDF_WITH_HTTP)) {
			...
			log.warn("Unknown configuration option for replacing relative URLs in JDF files. "
					+ "Using property value as base URL: " + replaceUrls);
			baseUrl = replaceUrls;
		}
		return startTestSessionWithSubmitQueueEntry(jdfFile, targetUrl, preprocessJdf, asMime,
				baseUrl);
	}

	/**
	 * Starts a new test session by resubmitting a JDF file. Any relative in the
	 * JDF file will be resolved against the JDF file and replaced with http
	 * URLs.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 * @param queueEntryId
	 *            the queue entry ID of the job to resubmit
	 * @param jobId
	 *            the /JDF/@JobID of the job to resubmit
	 * @param targetUrl
	 *            the URL to submit it to
	 * @return the test session
	 * @see #startTestSessionWithResubmitQueueEntry(File, String, String,
	 ...
	 */
	public TestSession startTestSessionWithResubmitQueueEntryWithHttpUrls(File jdfFile,
			String queueEntryId, String jobId, String targetUrl) {
		return startTestSessionWithResubmitQueueEntry(jdfFile, queueEntryId, jobId, targetUrl,
				getPublishedJDFBaseURL(), true, false);
	}

	/**
	 * Starts a new test session by resubmitting a JDF file. Any relative in the
	 * JDF file will be resolved against the JDF file and replaced with absolute
	 * file URLs.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 ...
	 * @see #publishJDFWithFileUrls(File, String)
	 */
	public TestSession startTestSessionWithResubmitQueueEntryWithFileUrls(File jdfFile,
			String queueEntryId, String jobId, String targetUrl) {
		return startTestSessionWithResubmitQueueEntry(jdfFile, queueEntryId, jobId, targetUrl,
				jdfFile.getAbsolutePath(), true, false);
	}

	/**
	 * Starts a new test session by sending a <i>ResubmitQueueEntry</i> JMF
	 * message. that refers to the specified JDF file.
	 * 
	 * If <code>asMime</code> is <code>true</code> the JDF file, its content
	 * files, and the <i>ResubmitQueueEntry</i> JMF message are bundled in a
	 * MIME package.
	 * 
	 * If <code>asMime</code> is <code>false</code>, the JDF job, the JDF
	 * file is published to this TestRunners HTTP server and the resulting http
	 * URL is used in the <i>ResubmitQueueEntry</i> JMF message. Any relative
	 * URLs in the JDF file will be resolved against <code>baseUrl</code> and
	 * replaced with an absolute URL, see
	 * {@link #publishJDF(File, String, String)}.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 * @param queueEntryId
	 ...
	 *            If the JDF file references any files using relative URLs then
	 *            these will be resolved against this base URL.
	 *            <code>null</code> to not resolve and replace any URLs.
	 * @param preprocessJdf
	 * @param asMime
	 *            packages the JDF file, its content files, and the
	 *            SubmitQueueEntry JMF in a MIME package
	 * @return the test session
	 */
	public TestSession startTestSessionWithResubmitQueueEntry(File jdfFile,
			String queueEntryId, String jobId, String targetUrl, String baseUrl,
			boolean preprocessJdf, boolean asMime) {
		jdfFile = publishJDF(jdfFile);
		PreprocessorContext context = new PreprocessorContext();
		context.addAttribute(JDFPreprocessor.PREPROCESSING_ENABLED, Boolean
		...
		context.addAttribute(UrlResolvingPreprocessor.BASEURL_ATTR, baseUrl);
		OutMessage message = JMFMessageBuilder.buildResubmitQueueEntry(
				resolvePublishedJDF(jdfFile), queueEntryId);
		return startTestSession(message, jdfFile, context, targetUrl, asMime);
	}
	 ...
	 * If <code>asMime</code> is <code>false</code>, the JDF job, the JDF
	 * file is published to this TestRunners HTTP server and the resulting http
	 * URL is used in the <i>ResubmitQueueEntry</i> JMF message.
	 * 
	 * @param jdfFile
	 *            the JDF file to submit
	 * @param queueEntryId
	 ...
	 */
	public TestSession startTestSessionWithResubmitQueueEntry(File jdfFile,
			String queueEntryId, String jobId, String targetUrl, boolean preprocessJdf,
			boolean asMime) {
		final String baseUrl;
		final String replaceUrls = _confHand.getProp(ConfigurationHandler.REPLACE_URLS_IN_JDF);
		if (replaceUrls.equals(ConfigurationHandler.REPLACE_URLS_IN_JDF_WITH_HTTP)) {
	...
	public File publishJDF(File jdfFile) {
		...
		if (!jdfFile.exists()) {
			throw new IllegalArgumentException("The JDF file '" + jdfFile.getAbsolutePath()
					+ "' cannot be published because the file does not exist.");
		}
		...
		try {
			...
			log.error("The JDF file '" + jdfFile + "' could not be published to '"
					+ publicJdfFile + "'.", e);
		}
	...
	/**
	 * Returns the HTTP URL of the directory where JDF files are published so
	 * that they can be downloaded.
	 * 
	 * @return the HTTP URL, ending with a slash character, where JDF files are
	 *         published
	 */
	...
	private OutMessage preprocessMIME(OutMessage message, PreprocessorContext context) {		
        ...
        if (mjmEnabled) {
            ...
            // glue it back
            JDFDoc jdfDoc = new JDFParser().parseFile(
                    TestSessionImpl.getJDFFileFromMime(message));
	...
	/**
	 * Applies all preprocessors in the List to the Message. Preprocessors are
	 * applied in the order they occur in the list. If a preprocessor fails it
	 * does so quietly and preprocessing continues.
	 * 
	 * @param preprocessors
	 * @param message
	 */
	...
	/**
	 * Preprocesses the JDF file with the currently configured preprocessors.
	 * The file is read from disk, preprocessed, and then written back to the
	 * same file. If a preprocessor fails it does so quietly and preprocessing
	 * continues. If the file could not be parsed a warning is logged and
	 * <code>null</code> is returned.
	 * 
	 * @param jdfFile
	 *            the JDF file to preprocess
	 * @param context
	 *            the preprocessing context
	 * @return the preprocessed JDF node that was written back to the file;
	 *         <code>null</code> if the JDF file could not be parsed
	 */
	private JDFNode preprocessJDF(File jdfFile, PreprocessorContext context) {

		JDFDoc jdfDoc = new JDFParser().parseFile(jdfFile.getAbsolutePath());
		if (jdfDoc == null) {
			log.warn("Could not parse JDF file '" + jdfFile.getAbsolutePath()
					+ "' for preprocessing.");
			return null;
		}
		...
		if (!jdf.getOwnerDocument_KElement().write2File(jdfFile.getAbsolutePath(), 2, true)) {
			log.warn("Could not write preprocessed JDF back to file '"
					+ jdfFile.getAbsolutePath() + "'.");
			return null;
		}
		return jdf;
	}
	...
	/**
	 * Preprocesses the JDF node with the currently configured preprocessors. If
	 * a preprocessor fails it does so quietly and preprocessing continues.
	 * 
	 * @param jdf
	 *            the JDF node to preprocess
	 * @param context
	 *            the preprocessing context
	 */
	...
	/**
	 * Serializes the test suite, all incoming and outgoing messages to a
	 * directory and creates an XML-based test report file containing a log of
	 * all messages and the test results.
	 * 
	 * @param outputDir
	 *            the directory to write the test suite to
	 * @return the XML-based test report file
	 * @throws IOException
	 */
	public synchronized String serializeTestSuite(String xsltDir, String outputDir)
			throws IOException {
		TestSuiteSerializer serializer = new TestSuiteSerializer(xsltDir);
	...
	private class PublishedFile extends File {
		...
		/**
		 * @param parent
		 *            the published file's parent path
		 * @param child
		 *            the published file's name
		 * @param originalFile
		 *            the original file
		 */
