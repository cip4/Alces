...
public class NodeInfoPreprocessor implements JDFPreprocessor {
	...
	/**
	 * Attribute for storing the <em>/JMF/Message/@ID</em> in the context. If
	 * this preprocessor finds this attribute in the context it will use it as a
	 * prefix for the IDs of the messages found in the <em>/JDF/NodeInfo</em>
	 * element.
	 */
	public final static String MESSAGEID_PREFIX_ATTR = "org.cip4.elk.alces.NodeInfoPreprocessor.MessageID";
	
	/**
	 * Attribute for storing the <em>/JMF/Message/Subscript/@URL</em> in the context. If
	 * this preprocessor finds this attribute in the context it will replace all existing
	 * subscription URLs with this attribute's value.
	 */
	public final static String SUBSCRIPTION_URL_ATTR = "org.cip4.elk.alces.NodeInfoPreprocessor.SubscriptionURL";

	private static Log log = LogFactory.getLog(NodeInfoPreprocessor.class);
		...
		for (JDFJMF jmf : jmfs) {			
			...
			for (JDFMessage msg : msgs) {				
				...
				if (context != null && context.getAttribute(MESSAGEID_PREFIX_ATTR) != null) {
					msg.setID(context.getAttribute(MESSAGEID_PREFIX_ATTR)
							+ "_" + factory.newMessageID());
				} else {
